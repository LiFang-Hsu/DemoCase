...
(function (f, define) {
    ...
    var __meta__ = {
        ...
        name: 'TWDatePicker',
        category: 'web',
        description: 'The TWDatePicker widget allows the user to select a date from a calendar or by direct input.',
        depends: [
            'twcalendar',
            'popup'
        ]
    ...
    (function ($, undefined) {
        var kendo = window.kendo, ui = kendo.ui, Widget = ui.Widget, parse = kendo.parseDate, keys = kendo.keys, template = kendo.template, activeElement = kendo._activeElement, DIV = '<div />', SPAN = '<span />', ns = '.kendoTWDatePicker', CLICK = 'click' + ns, OPEN = 'open', CLOSE = 'close', CHANGE = 'change', DISABLED = 'disabled', READONLY = 'readonly', DEFAULT = 'k-state-default', FOCUSED = 'k-state-focused', SELECTED = 'k-state-selected', STATEDISABLED = 'k-state-disabled', HOVER = 'k-state-hover', HOVEREVENTS = 'mouseenter' + ns + ' mouseleave' + ns, MOUSEDOWN = 'mousedown' + ns, ID = 'id', MIN = 'min', MAX = 'max', MONTH = 'month', ARIA_DISABLED = 'aria-disabled', ARIA_EXPANDED = 'aria-expanded', ARIA_HIDDEN = 'aria-hidden', ARIA_READONLY = 'aria-readonly', calendar = kendo.twcalendar, isInRange = calendar.isInRange, restrictValue = calendar.restrictValue, isEqualDatePart = calendar.isEqualDatePart, extend = $.extend, proxy = $.proxy, DATE = Date, objectToString = {}.toString;
        function normalize(options) {
            var parseFormats = options.parseFormats, format = options.format;
            calendar.normalize(options);
            parseFormats = $.isArray(parseFormats) ? parseFormats : [parseFormats];
            if (!parseFormats.length) {
                parseFormats.push('yyyyMMdd');
        ...
        DateView.prototype = {
            _calendar: function () {
                ...
                if (!calendar) {
                    ...
                    that.calendar = calendar = new ui.TWCalendar(div);
        ...
        var DatePicker = Widget.extend({
            init: function (element, options) {
                ...
                options.disableDates = kendo.twcalendar.disabled(options.disableDates);
            ...
            options: {
                ...
                month: {},
                dates: [],
                ARIATemplate: 'Current focused date is #=kendo.formatTWDate(data.current, "D")#'
            },
            setOptions: function (options) {
                var that = this;
                ...
                if (value) {
                    that.element.val(kendo.formatTWDate(value, options.format, options.culture));
            ...
            _update: function (value) {
                var that = this, options = that.options, min = options.min, max = options.max, current = that._value, value = parseTWDateToADDate(value), date = parse(value, options.parseFormats, options.culture), isSameType = date === null && current === null || date instanceof Date && current instanceof Date, formattedValue;
                ...
                if (+date === +current && isSameType) {
                    formattedValue = kendo.formatTWDate(date, options.format, options.culture);
                ...
                if (date !== null && isEqualDatePart(date, min)) {
                    ...
                    date = null;
                    // 2018/12/07 加上此設定，避免手動輸入不在區間內日期，卻在畫面顯示
                    value = null;
                }
                that._value = date;
                that.dateView.value(date);
                that.element.val(date ? kendo.formatTWDate(date, options.format, options.culture) : value);
                that._updateARIA(date);
                return date;
            },
            _wrapper: function () {
